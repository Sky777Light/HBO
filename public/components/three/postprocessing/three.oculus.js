...
THREE.OculusRiftEffect = function ( renderer, options ) {
    ...
    var RTMaterial = new THREE.ShaderMaterial( {
        ...
        fragmentShader: [
            ...
            "  float alpha = (texture2D(texid, tcRed).a + texture2D(texid, tcGreen).a + texture2D(texid, tcBlue).a) / 3.0;",
            "  gl_FragColor = vec4(texture2D(texid, tcRed).r, texture2D(texid, tcGreen).g, texture2D(texid, tcBlue).b, alpha);",
            "}"
        ].join("\n")
    } );
    ...
    this.setHMD = function(v) {
        ...
        renderTarget = new THREE.WebGLRenderTarget( 1350, 1350, RTParams );
    ...
    this.render = function ( scene, camera ) {
        ...
        // Clear
        //renderer.setClearColor(emptyColor, 0.0);
        renderer.clear();
        renderer.setClearColor(cc, 0.0);
